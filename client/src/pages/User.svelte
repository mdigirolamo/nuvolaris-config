...
<section class="p-8 max-w-lg mx-auto">
  ...
  {#if apiStatusCode === APIStatusCode.OK && !isBusy}
    <span class="inline-block text-green-600 pb-2" transition:slide="{{ duration: 250 }}">
  ...
  {:else if apiStatusCode === APIStatusCode.ALREADY_EXISTS && !isBusy}
    <span class="inline-block text-red-600 pb-2" transition:slide="{{ duration: 250 }}">
  ...
  <UserProfileForm on:form-data="{onFormData}" {isBusy}>
    <svelte:fragment slot="submit-button-spinner">
